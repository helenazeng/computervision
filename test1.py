...
test = []
for j in range(0,2):
    test.append(mh.imread('eecs442challenge/test/color/'+str(j)+'.png'))

for i in range(0,20):
    color.append(mh.imread('eecs442challenge/train/color/'+str(i)+'.png'))
 #   mask.append(mh.imread('eecs442challenge/train/mask/'+str(i)+'.png'))
    normal.append(mh.imread('eecs442challenge/train/normal/'+str(i)+'.png'))
...
#network = conv_2d(network, 128, 3, activation='relu') # size 128*128*128

#network = max_pool_2d(network, 2, strides=2)  #size 64*64*128
...
model = tflearn.DNN(network, checkpoint_path='none',
                    max_checkpoints=0, tensorboard_verbose=2)
...
model.fit(X, Y, n_epoch=10, shuffle=True,
          show_metric=True, batch_size=100, snapshot_step=500,
